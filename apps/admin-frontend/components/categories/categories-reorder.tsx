...
import { GripVertical, Save, FolderTree } from "lucide-react";
...
export function CategoriesReorder({ items, onSave, isLoading }: CategoriesReorderProps) {
  const [orderedItems, setOrderedItems] = React.useState<Category[]>(items);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between px-2">
        <p className="text-sm text-muted-foreground">Drag to rearrange the display order for the storefront architecture.</p>
        <Button 
            onClick={handleSave} 
            disabled={isLoading} 
            size="sm" 
