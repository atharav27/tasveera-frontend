...
import { Hamper } from "./mock-data";
import { format } from "date-fns";

interface HampersTableProps {
  data: Hamper[];
  pageCount: number;
...
export function HampersTable({ 
  ...
  const columns = React.useMemo<ColumnDef<Hamper>[]>(
    () => [
      {
        accessorKey: "id",
        header: ({ column }) => (
        ...
        cell: ({ row }) => (
          <button
            type="button"
            onClick={() => router.push(`/catalog/hampers/${row.original.id}`)}
      ...
      },
      {
        id: "actions",
        cell: ({ row }) => {
          const hamper = row.original;
          const items: ActionItem[] = [
            {
              label: "View / Edit",
              icon: "pencil",
              onClick: () => router.push(`/catalog/hampers/${hamper.id}`),
            },
            {
              label: hamper.isActive ? "Deactivate" : "Activate",
              icon: hamper.isActive ? "xcircle" : "circleCheckBig",
              onClick: () => onToggleActive(hamper.id),
            },
            {
              label: "Delete Hamper",
              icon: "trash2",
              variant: "destructive",
              onClick: () => setIsDeleting(hamper.id),
            },
          ];
  ...
  return (
    <>
        ...
        <CommonAlertDialog
            ...
            title="Delete Hamper"
            subtitle="Are you sure you want to delete this hamper? Action cannot be undone."
            variant="error"
